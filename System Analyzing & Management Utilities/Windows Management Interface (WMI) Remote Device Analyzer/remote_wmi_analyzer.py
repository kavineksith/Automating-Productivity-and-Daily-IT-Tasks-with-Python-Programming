...
For Remote Located Device Analyzing Script of Windows Management Interface (WMI)
...
class AuthenticationError(WmiError):
    """Exception raised when authentication to a remote system fails"""
    def __init__(self, message):
        self.message = message
        super().__init__(self.message)
...
class WmiSystemInfo:
    def __init__(self, target_computer=None, username=None, password=None):
        """
        Initialize WMI System Information class
        
        Args:
            target_computer (str, optional): Remote computer name. Defaults to None (local).
            username (str, optional): Username for remote connection. Defaults to None.
            password (str, optional): Password for remote connection. Defaults to None.
        """
        self.logger = setup_logger()
        self.target_computer = target_computer
        
        # Set connection type info for logging
        conn_type = "remote" if target_computer else "local"
        self.logger.info(f"Initializing WMI System Information for {conn_type} connection")
        
        try:
            # Establish WMI connection based on parameters
            if target_computer:
                if not username or not password:
                    raise AuthenticationError("Username and password required for remote connection")
                self.logger.info(f"Connecting to remote computer: {target_computer}")
                self.c = wmi.WMI(computer=target_computer, user=username, password=password)
                self.logger.info(f"Successfully connected to remote computer: {target_computer}")
            else:
                self.logger.info("Establishing local WMI connection")
                self.c = wmi.WMI()
                self.logger.info("Local WMI connection established")
                
        except AuthenticationError as e:
            self.logger.critical(f"Authentication error: {str(e)}")
            raise
        except Exception as e:
            self.logger.critical(f"Failed to connect to WMI: {str(e)}")
            if target_computer:
                raise ConnectionError(f"Could not establish WMI connection to {target_computer}: {str(e)}")
            else:
                raise ConnectionError(f"Could not establish local WMI connection: {str(e)}")
    ...
    def collect_all(self):
        ...
        target_info = f"from {self.target_computer}" if self.target_computer else "from local machine"
        self.logger.info(f"Starting collection of all WMI information {target_info}")
        results = {}
        ...
        self.logger.info(f"Completed collection of all WMI information {target_info}")
        return results
    ...
    def collect_specific(self, collector_names):
        ...
        """
        target_info = f"from {self.target_computer}" if self.target_computer else "from local machine"
        self.logger.info(f"Starting collection of specific WMI information {target_info}: {collector_names}")
        results = {}
        ...
        self.logger.info(f"Completed collection of specific WMI information {target_info}")
        return results
    ...
    def manage_service(self, service_name, action):
        ...
        Returns:
            bool: Success status
        """
        target_info = f"on {self.target_computer}" if self.target_computer else "on local machine"
        self.logger.info(f"Managing service {service_name} with action {action} {target_info}")
...
def main():
    ...
    """
    # Set up logging using the existing setup_logger function
    logger = setup_logger()
    ...
    # Add remote connection options to all parsers
    for subparser in [all_parser, specific_parser, service_parser]:
        subparser.add_argument('--remote', help='Remote computer name')
        subparser.add_argument('--username', help='Username for remote connection')
        subparser.add_argument('--password', help='Password for remote connection')
    ...
    try:
        # Determine if connecting to local or remote machine
        if hasattr(args, 'remote') and args.remote:
            if not args.username or not args.password:
                logger.error("Username and password are required for remote connections")
                print("Error: Username and password are required for remote connections")
                return 1
                
            logger.info(f"Connecting to remote machine: {args.remote}...")
            print(f"Connecting to remote machine: {args.remote}...")
            wmi_info = WmiSystemInfo(args.remote, args.username, args.password)
            logger.info(f"Successfully connected to remote machine: {args.remote}")
        else:
            # Create local WmiSystemInfo instance
            logger.info("Using local WMI connection")
            wmi_info = WmiSystemInfo()
            logger.info("Successfully initialized local WMI connection")
       
        # Initialize results dictionary
        results = {"status": "success", "data": {}, "errors": []}
        
        # Add connection info to results
        results["connection"] = "remote" if hasattr(args, 'remote') and args.remote else "local"
        if hasattr(args, 'remote') and args.remote:
            results["remote_machine"] = args.remote
